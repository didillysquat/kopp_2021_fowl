...
bcftools_call_publishDir = [params.output_dir, "bcftools_vcf"].join(File.separator)
...
if (params.mode == "bcftools"){
    ...
    // gatk_haplotype_caller_gvcf_ch.collect{it[1]}.view()
    process bcftools_mpileup{
        container 'halllab/bcftools:v1.9'

        input:
        each scaffold from Channel.fromList(scaffold_list)
        tuple path(ref_genome), path(ref_genome_dict), path(ref_genome_fai) from gatk_haplotype_caller_ref_genome_ch
        path(bam_bai) from gatk_haplotype_caller_gvcf_ch.collect{it[1]}
    ...
    process gather_vcfs_for_eval{
        ...
        tuple path("out.vcf.gz"), path("out.vcf.gz.tbi") into gather_vcfs_out_ch
    ...
    throw new Exception("""Genotyping mode must be provided: \"--mode gatk\" or \"--mode bcftools\"""")
}
