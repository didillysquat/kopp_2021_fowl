...
if (params.mode == "bcftools"){
    ...
    input_bam_ch.into{bcftools_mpileup_ch}
...
process bcftools_vcfstats{
    ...
    """
    bcftools stats -F $ref_genome -s - $vcf > $stats_out_file
    """
}
...
process rtg_vcfstats_per_sample{
    ...
    script:
    stats_out_file = vcf.getName().replaceAll(".vcf", ".rtg.stats.per_sample.txt")
    """
    rtg vcfstats $vcf > $stats_out_file
    """
}
