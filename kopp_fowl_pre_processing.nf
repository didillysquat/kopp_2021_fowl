...
// The values used are taken from the user input params.read_info_file file. This is a tab
// delimited file that has the columns:
// file_name       RGID    RGLB    RGPL    RGPU    RGSM
// For each file name, we export the pair_id and store the string that will be used for the 
// AddOrReplaceReadGroups command.
// A unique sample name must be given to each pair_id indepent of read group.
